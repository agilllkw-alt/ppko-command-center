...
<head>
    ...
    <!-- Meta untuk WhatsApp/FB Preview -->
    <meta property="og:title" content="CARA'DE Command Center">
    <meta property="og:description" content="Sistem Informasi Digital Desa Tinggimae - PPKO 2026">
    <meta property="og:image" content="https://raw.githubusercontent.com/agilllkw-alt/ppko-command-center/main/logo-tim.png">
...
<!-- 2. HEADER DENGAN LOGO LENGKAP -->
<header class="sticky top-0 z-[100] glass border-b border-green-200 px-4 py-3 shadow-sm">
    <div class="max-w-7xl mx-auto flex flex-col gap-4">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-1.5 bg-white/60 p-2 rounded-2xl border border-white">
                <!-- DERETAN LOGO LENGKAP -->
                <img src="logo-unhas.png" alt="UNHAS" class="h-6 w-auto">
                <img src="logo-bem.png" alt="BEM" class="h-6 w-auto">
                <img src="logo-tim.png" alt="TIM" class="h-6 w-auto">
                <img src="logo-gowa.png" alt="GOWA" class="h-6 w-auto">
                
                <div class="w-[1px] h-6 bg-green-200 mx-1"></div>
                <div class="flex flex-col">
                    <span class="font-['Space_Grotesk'] text-[11px] font-bold text-green-900 uppercase italic leading-none">CARA'DE</span>
                    <span class="text-[8px] text-red-600 font-bold leading-none mt-1">ᨌᨑᨉᨙ</span>
                </div>
            </div>
...
<main id="tab-modul" class="tab-content active w-full px-4 py-6 max-w-7xl mx-auto">
    ...
    <div id="module-display" class="bg-white rounded-[2.5rem] p-8 shadow-2xl border border-green-100 min-h-[400px]"></div>
</main>
...
<main id="tab-cuaca" class="tab-content w-full px-4 py-6 max-w-4xl mx-auto space-y-6">
    <div class="weather-card text-white p-6 rounded-[2.5rem] shadow-xl">
        ...
        <div class="grid grid-cols-3 gap-4 mt-8 pt-6 border-t border-white/20 text-center">
            <div><p class="text-[8px] opacity-70 mb-1">Kelembapan</p><p id="weather-hum" class="font-bold">--%</p></div>
            <div><p class="text-[8px] opacity-70 mb-1">Angin</p><p id="weather-wind" class="font-bold">-- km/h</p></div>
            <div><p class="text-[8px] opacity-70 mb-1">Indeks UV</p><p id="weather-uv" class="font-bold">--</p></div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2.5rem] p-6 shadow-xl border border-green-100">
        ...
        <div id="ai-chat-box" class="ai-chat-box flex flex-col no-scrollbar mb-4">
            <div class="message ai-msg">Halo! Saya AI Cara'de. Ada yang bisa saya bantu terkait 5 modul kami?</div>
        </div>
        <div class="flex gap-2 bg-slate-50 p-1.5 rounded-full border border-slate-200">
            <input id="ai-input" type="text" placeholder="Tanya tentang pakan, maggot, kincir..." class="flex-1 bg-transparent px-4 text-[11px] outline-none italic">
...
<main id="tab-tim" class="tab-content w-full px-4 py-6 max-w-7xl mx-auto">
    <div class="mb-8 bg-green-900 text-white p-6 rounded-[2.5rem] shadow-xl">
        <p class="text-[10px] text-green-400 font-black uppercase italic">Dosen Pendamping</p>
        <h3 class="text-xl font-black italic tracking-tighter">Husnul Mubarak, S.TP., M.Si</h3>
        <p class="text-[8px] opacity-60">Universitas Hasanuddin (UNHAS)</p>
    </div>
...
<footer class="py-16 text-center opacity-30 text-[9px] font-black uppercase tracking-[1em]">
    CARA'DE // 2026
</footer>
...
<script>
    // DATA MODUL
    const mods = {
        1: { t: "APPARE'", l: "ᨕᨄᨑᨙ", s: "Teknologi Pakan", i: "⚙️", d: "Mesin produksi pakan mandiri yang memanfaatkan limbah lokal desa untuk meningkatkan efisiensi peternakan.", keywords: ["pakan", "mesin", "ternak"] },
        2: { t: "MAGGOT", l: "ᨑᨁᨚ", s: "Protein Alternatif", i: "🪱", d: "Budidaya Maggot BSF (Black Soldier Fly) sebagai pengurai sampah organik sekaligus sumber protein tinggi pakan.", keywords: ["maggot", "bsf", "sampah", "protein"] },
        3: { t: "PANGE'BA", l: "ᨄᨂᨙᨅ", s: "Otomasi Aerasi", i: "🌊", d: "Sistem kincir air otomatis berbasis sensor untuk menjaga kadar oksigen terlarut dalam kolam budidaya.", keywords: ["kincir", "oksigen", "kolam", "aerasi"] },
        4: { t: "PATTAPPARANG", l: "ᨄᨈᨄᨑ", s: "Smart Greenhouse", i: "🌿", d: "Monitoring iklim mikro (suhu & kelembapan) berbasis IoT untuk optimalisasi hasil pertanian dalam rumah kaca.", keywords: ["iot", "greenhouse", "tanaman", "suhu"] },
        5: { t: "PA'BULOANG", l: "ᨄᨅᨘᨒᨚᨕ", s: "Bisnis Digital", i: "📈", d: "Strategi pemasaran digital, branding produk desa, dan manajemen keuangan untuk keberlanjutan ekonomi.", keywords: ["bisnis", "marketing", "digital", "ekonomi"] }
    };
    ...
    // INITIALIZATION
    window.onload = () => {
        // Hilangkan Splash Screen secara otomatis setelah 2.5 detik
        setTimeout(() => {
    ...
    function setModule(id) {
        ...
        document.getElementById('module-display').innerHTML = `
            <div class="flex flex-col items-center text-center animate-fadeIn">
                <div class="text-[90px] mb-2">${m.i}</div>
                ...
                <button onclick="window.open('modul${id}.pdf', '_blank')" class="w-full bg-green-700 text-white py-5 rounded-[2.5rem] font-black text-[11px] uppercase italic shadow-xl active:scale-95 transition">Buka Dokumen Modul PDF</button>
            </div>
        `;
    }
    ...
    function renderTeam() {
        document.getElementById('team-grid').innerHTML = members.map(m => `
            <div class="glass p-4 rounded-3xl border-b-4 border-green-200">
                <h4 class="text-[9px] font-black uppercase text-green-950 truncate italic">${m.n}</h4>
                <p class="text-[7px] font-bold text-green-600 mt-1 opacity-60">${m.nim}</p>
            </div>
        `).join('');
    }

    // FUNGSI CUACA (Tinggimae)
    async function fetchWeather() {
        try {
            ...
            const codes = { 0: "Cerah", 1: "Cerah Berawan", 2: "Berawan", 3: "Mendung", 61: "Hujan Ringan", 95: "Badai Petir" };
        ...
        } catch (e) { document.getElementById('live-desc').innerText = "Cek Koneksi..."; }
    }
    ...
    async function askAI() {
        ...
        inp.value = '';
        setTimeout(() => {
            let res = "Saya AI Cara'de. Tanya tentang pakan, maggot, kincir, IoT, atau bisnis digital!";
            Object.values(mods).forEach(m => {
                if(m.keywords.some(k => query.includes(k))) res = `Terkait <b>${m.t}</b>: ${m.d}`;
            });
            if(query.includes("tinggimae")) res = "Desa Tinggimae terletak di Kec. Somba Opu, Gowa, tempat kami mengabdi.";
            addMsg(res, 'ai-msg');
        }, 800);
    }
    ...
    function showTab(id) {
        ...
        if(id === 'tab-lokasi') setTimeout(initMap, 500);
